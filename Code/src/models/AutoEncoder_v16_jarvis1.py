...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, MaxPooling2D, Add, \
                                    ...
                                    SpatialDropout2D, Activation, GaussianNoise
###################################################################################################
'''
MODEL DEFINITION:
Simple Autoencoder
...
def BridgeBlock(img, **conv_args):
    ...
    ######################
    # Signal Transmission
    out = GaussianNoise(stddev=0.1)(out)
    ######################
    shortcut_2 = Conv2D_BatchNorm(out, **conv_args)
    out = ResBlock(out, **kwargs)
    out = ResBlock(out, **kwargs)
    out = Add()([out, shortcut_2])
    # kwargs = conv_args.copy()
    # kwargs['filters'] //= 2
    # kwargs['kernel_size'] = 1
